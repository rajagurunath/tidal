...
Three implementation observations from bringing the system up are worth recording. First, the adapter exercise validates the wire-compatibility claim beyond our own code: lazycode's Tidal adapter mirrors its Anthropic one nearly line-for-line, and the friction that remains is OpenAI's own protocol, not ours — the Batch API's `request_counts` lacks an `expired` field (the adapter must infer expired items from the batch status and the count remainder), results require an extra retrieve round-trip plus two file downloads where Anthropic streams one result set, and cancelled items are silently absent from both output files rather than individually reported. Anyone building batch clients inherits these quirks from the de-facto standard; we chose fidelity to them over cleanliness. Second, an engine plugin has an observability problem a fork does not: vLLM's logging configuration attaches handlers only to its own logger tree, and scheduler telemetry is emitted from a forked EngineCore process — without injecting a logging configuration, technique B runs blind. Third, escalation applied at submission time interacts with cold start: a freshly created batch has no observed completion rate, and a naive laxity computation with an epsilon-floored rate makes every new batch look infinitely late. Tidal drops the drain term until completions exist — batches escalate on evidence or deadline proximity, never on ignorance.
...
**Admission gating does not bound resident-request inflation.** Technique B's pre-gate caps how much batch work *enters* the engine, but a batch request already resident keeps decoding every iteration until the KV guardband evicts it — and on a machine where KV pressure never materializes, eviction never fires. On our CPU testbed this is the dominant residual interference (resident batch decodes inflating online steps well beyond what the admission cap suggests); bounding it requires either per-iteration participation control (reimplementing the scheduler's inner loop, abandoning the wrapper's upstream-tolerance) or ConServe-style sub-iteration preemption. We report it as the honest price of the plugin constraint.

**Tail metrics flatter co-location.** In our measurements the online *median* degrades more than the p99: co-serving adds a roughly constant per-step cost that dominates fast requests while the tail was already slow. Evaluations in this space that report only p99 (common) overstate how invisible co-location is; we report full distributions.

Single engine, single model; multi-replica routing is future work. A single dispatcher per store is assumed (no ownership lease; the store's idempotent terminal transitions make a violation safe for data but not for duplicated work). Submission-time escalation cannot re-age items already in the engine queue (bounded impact: the backlog dominates laxity, and queue residence is short by construction). The chunk-granularity cap is coarser than per-token enforcement. The step-time signal on GPU builds is complicated by async scheduling; our timing attribution is exact on the CPU build and approximate otherwise. `scheduler_cls` is explicitly not a stable vLLM interface; our wrapper touches a three-method surface and we pin a version, but upstream can break it. The 50% price is inherited from the commercial anchor, not derived from a cost model — deriving the profit-maximizing discount from measured marginal cost is an open question we'd like to answer with production traces.
